...
struct VertexArray {
    ...
    func getVertexIndexCInt(x: Int, _ y: Int) -> CInt {
        return CInt(getVertexIndex(x, y))
    }
    func getVertex(x: Int, _ y: Int) -> Float3 {
        return array[getVertexIndex(x, y)]
    }
    func getAdjacentVertex(x: Int, y: Int, from direction: Direction) -> Float3? {
        let coordinate = getAdjacentVertexCoordinate(x, y: y, from: direction)

        if coordinate.0 >= 0 &&
            coordinate.0 < width &&
            coordinate.1 >= 0 &&
            coordinate.1 < height {
            return getVertex(coordinate.0, coordinate.1)
        }
        return nil
    }
    ...
    func getAdjacentVertexIndexCInt(x: Int, y: Int, from direction: Direction) -> CInt {
        return CInt(getAdjacentVertexIndex(x, y: y, from: direction))
    }
    func getCrossProduct(x: Int, y: Int, from planarDirection: PlanarDirection) -> Float3? {
        ...
        if let vNotNil = v {
            if let uNotNil = u {
                ...
                if continuesInX {
                    return toV.crossProductWith(toU)
                }
    ...
    func getNormal(x: Int, y: Int) -> Float3 {
        let crossNorthWest = getCrossProduct(x, y: y, from: .NorthWest)
        let crossNorthEast = getCrossProduct(x, y: y, from: .NorthEast)
        let crossSouthEast = getCrossProduct(x, y: y, from: .SouthEast)
        let crossSouthWest = getCrossProduct(x, y: y, from: .SouthWest)
        ...
        var sum = Float3(x: 0, y: 0, z: 0).add(crossNorthWest).add(crossNorthEast).add(crossSouthEast).add(crossSouthWest)
        
        return sum.normalize()
    }
    mutating func setVertex(vertex: Float3, x: Int, y: Int) {
        array[getVertexIndex(x, y)] = vertex
    }
}